...
namespace rem
{
	...
	Help::Help(QWidget *parent) : QDialog(parent)
	{
		...
		//加载按钮样式
		ui.pushButton_close_dialog->setStyleSheet("QPushButton{border-image: url(:/picture_tools/icon/picture_tools/icon/dialog_main_exit_normal.png);}""QPushButton:hover{border-image: url(:/picture_tools/icon/picture_tools/icon/dialog_main_exit_pressed.png);}");
		...
		//信号函数
		connect(ui.pushButton_close_dialog, SIGNAL(clicked()), this, SLOT(close_window_include_animation()));//关闭按钮
