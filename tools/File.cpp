...
namespace rem
{
	...
	File::File(QWidget *parent) : QDialog(parent)
	{
		...
		//加载按钮样式
		ui.pushButton_exit->setStyleSheet("QPushButton{border-image: url(:/picture_tools/dialogicon/picture_tools/icon/dialog_filedialog_exit_normal.png);}""QPushButton:hover{border-image: url(:/picture_tools/dialogicon/picture_tools/icon/dialog_filedialog_exit_pressed.png);}");
		...
		connect(ui.pushButton_close_window, SIGNAL(clicked()), this, SLOT(close_window_include_animation()));//关闭按钮
		connect(ui.pushButton_exit, SIGNAL(clicked()), this, SLOT(close_window_include_animation()));//关闭按钮
	...
	void File::set_button_save_or_open(int status)//设置按钮
	{
		if (status == 0)
		{
			ui.pushButton_open->setStyleSheet("QPushButton{border-image: url(:/picture_tools/dialogicon/picture_tools/icon/dialog_filedialog_open_normal.png);}""QPushButton:hover{border-image: url(:/picture_tools/dialogicon/picture_tools/icon/dialog_filedialog_open_pressed.png);}");
		}
		else if (status == 1)
		{
			ui.pushButton_open->setStyleSheet("QPushButton{border-image: url(:/picture_tools/dialogicon/picture_tools/icon/dialog_filedialog_save_normal.png);}""QPushButton:hover{border-image: url(:/picture_tools/dialogicon/picture_tools/icon/dialog_filedialog_save_pressed.png);}");
		}
		else
		{
	...
	void File::set_current_path(QString object_string)//传入当前工作路径
	{
		...
		extern QString font_str;//因为treeView内部的控件字体无法使用默认字体设置，因此必须单独设置。注意因为没有命名空间的关系，这个在main函数外面定义的全局变量不能在外面进行定义，否则会被当成一个新定义的全局变量
		if (font_str != "")
		{
	...
	void File::get_tree_view_selected_file_name(const QModelIndex &index)//文件路径。QT读取ini文件会自动进行转换，不需要替换"/"或者"\\"。但是本窗口的文本框显示必须转换
	{
		*file_path = file_tree->filePath(index);
		file_path_show = *file_path;
		file_path_show = (file_path_show).replace(tr("/"), tr("\\"));
		ui.textEdit_name->setText(file_path_show);
	}

	void File::get_text_edit_file_name()//获取并修改文件名
	{
	...
	File_only_path::File_only_path()
	{
		...
		connect(ui.textEdit_name, SIGNAL(textChanged(const QString &)), this, SLOT(get_text_edit_file_name()));//信号函数设置，用于文本框改变时获得文件路径
		connect(ui.pushButton_open, SIGNAL(clicked()), this, SLOT(open_or_save_button()));//打开或者保存按钮
		connect(ui.treeView_file, SIGNAL(doubleClicked(QModelIndex)), this, SLOT(open_or_save_button()));//双击树状图分支实现和按下打开或保存按钮同样的功能
		connect(ui.pushButton_close_window, SIGNAL(clicked()), this, SLOT(close_window_include_animation()));//关闭按钮
		connect(ui.pushButton_exit, SIGNAL(clicked()), this, SLOT(close_window_include_animation()));//关闭按钮
	...
	//设置成员变量
	void File_only_path::set_button_save_or_open(int status)//设置按钮
	{
		ui.pushButton_open->setStyleSheet("QPushButton{border-image: url(:/picture_tools/dialogicon/picture_tools/icon/dialog_filedialog_open_normal.png);}""QPushButton:hover{border-image: url(:/picture_tools/dialogicon/picture_tools/icon/dialog_filedialog_open_pressed.png);}");
	}

	void File_only_path::get_tree_view_selected_file_name(const QModelIndex &index)//文件路径。QT读取ini文件会自动进行转换，不需要替换"/"或者"\\"。但是本窗口的文本框显示必须转换
	{
		*file_path = file_tree->filePath(index);
		file_path_show = *file_path;
		...
		{
			file_path_show = file_information.absoluteDir().absolutePath();//只取文件路径
		}
		else
		{
