...
namespace rem
{
	...
	//构造与析构函数
	LockSimpleOpposite::LockSimpleOpposite(QWidget *parent) : QDialog(parent)
	{
		...
		ui.spinBox_3->setPalette(global_text_palette);
	...
	LockSimpleOpposite::~LockSimpleOpposite()
	{
		delete timer;
		delete effect_1;
		delete effect_2;
	...
	//设置成员函数
	void LockSimpleOpposite::update_unlock_information(int direction, long long head_offset, long long tail_offset, long long cycle, QString head_digit, bool have_head_digit)
	{
		...
		digit_array[2] = cycle;//循环周期
		ui.spinBox_3->setValue(cycle);
	...
	void LockSimpleOpposite::mousePressEvent(QMouseEvent *event)
	{
	...
	//若鼠标左键被按下，则移动窗体位置
	void LockSimpleOpposite::mouseMoveEvent(QMouseEvent *event)
	{
		if (m_bPressed)
		{
	...
	//鼠标未被按下，则相应变量为false
	void LockSimpleOpposite::mouseReleaseEvent(QMouseEvent *event)
	{
		Q_UNUSED(event);
		m_bPressed = false;
	}
	...
	void LockSimpleOpposite::keyPressEvent(QKeyEvent *event)
	{
	...
	void LockSimpleOpposite::timer_timeout()//使用2个label用来容纳图片。首先指定一个长时间间隔用来显示图片，此时图片在下层label，上层label使用全透明图片，即将其忽略。时间到期后，改用切换图片透明度所使用的短时间间隔，同时开始修改透明度计数，下层label显示原始图片，不透明度逐渐减小；上层label显示切换后图片，不透明度逐渐增加。当上层label图片透明度为1时，切换结束，图片计数增加1，以新的长时间间隔重新开始下一计时循环
	{
	...
	void LockSimpleOpposite::close_window_include_animation()//包含关闭窗口动画的关闭窗口方法
	{
	...
	void LockSimpleOpposite::lock_file()//加密文件
	{
		...
		if (file_information.isFile() && *change_result_address)//该文件存在
		{
			...
			digit_array[2] = ui.spinBox_3->value();//循环周期
			Lock_file_thread *thread = new Lock_file_thread();//处理文件子线程
			connect(thread, &QThread::finished, thread, &QObject::deleteLater);//直接delete子线程极其容易出现各种问题，要想安全删除线程可以使用此方式
			connect(this, SIGNAL(send_data(string*, bool*, long long*, int*, char*)), thread, SLOT(get_data(string*, bool*, long long*, int*, char*)));
			...
			emit send_data(string_array, bool_array, digit_array, flag_array, char_array);//发送数据
			...
			if (dll_name != NULL)
			{
				...
				if (get_class != NULL)
				{
					...
					connect(target_class_object, SIGNAL(lock_simple_opposite_process(double)), this, SLOT(receive_lock_simple_opposite_process(double)));//该函数要求的条件：发送者需要继承QObject类并且类中含有Q_OBJECT宏
					connect(target_class_object, SIGNAL(lock_simple_opposite_final(bool)), this, SLOT(receive_lock_simple_opposite_final(bool)));
				}
				else
				{
	...
	void LockSimpleOpposite::unlock_file()//解密文件
	{
		File dialog;//这个窗口是从Notepad.exe直接引用的，没有进行任何修改，因此File类的调用方式会比较麻烦
		dialog.setWindowTitle(tr("选择文件"));
		QString file_dialog_file_name = QDir::currentPath();//当前路径，用于获得被打开的文件路径
		...
		if (file_information.isFile() && *change_result_address)//该文件存在
		{
			...
			digit_array[1] = ui.spinBox_2->value();//尾部处理起点
			digit_array[2] = ui.spinBox_3->value();//循环周期
			UnlockSimpleOppositeCheck dialog;//创建确认解密信息框
			is_continue = false;//设置是否需要在下一窗口执行完后继续操作
			dialog.set_information(&flag_array[0], &digit_array[0], &digit_array[1], &digit_array[2], &lineEdit_text, &bool_array[3], &is_continue);//设定初始值
			dialog.exec();
			if (is_continue)//在解密确认窗口点击确认，接下来进行解密操作
			{
				update_unlock_information(flag_array[0], digit_array[0], digit_array[1], digit_array[2], lineEdit_text, bool_array[3]);//更新窗口内容
				Unlock_file_thread *thread = new Unlock_file_thread;//处理文件子线程
				connect(thread, &QThread::finished, thread, &QObject::deleteLater);//直接delete子线程极其容易出现各种问题，要想安全删除线程可以使用此方式
				connect(this, SIGNAL(send_data(string*, bool*, long long*, int*, char*)), thread, SLOT(get_data(string*, bool*, long long*, int*, char*)));
				connect(this, SIGNAL(send_target_class_point(Export_LockSimple_virtual_class *)), thread, SLOT(get_target_class_point(Export_LockSimple_virtual_class *)));
				connect(this, SIGNAL(send_dll_name(HINSTANCE)), thread, SLOT(get_dll_name(HINSTANCE)));
				...
				if (dll_name != NULL)
				{
					...
					if (get_class != NULL)
					{
						Export_LockSimple_virtual_class *target_class_object = get_class();//获得目标类的对象指针
						emit send_target_class_point(target_class_object);//发送dll导出类对象指针
						connect(target_class_object, SIGNAL(unlock_simple_opposite_process(double)), this, SLOT(receive_unlock_simple_opposite_process(double)));//该函数要求的条件：发送者需要继承QObject类并且类中含有Q_OBJECT宏
						connect(target_class_object, SIGNAL(unlock_simple_opposite_final(bool)), this, SLOT(receive_unlock_simple_opposite_final(bool)));
					}
					else
					{
	...
	void LockSimpleOpposite::receive_lock_simple_opposite_process(double index)//取反加密文件进度条显示。参数为已处理部分比例
	{
	...
	void LockSimpleOpposite::receive_lock_simple_opposite_final(bool is_success)//取反加密文件结束。参数为dll导出类指针
	{
	...
	void LockSimpleOpposite::receive_unlock_simple_opposite_process(double index)//取反加密文件进度条显示。参数为已处理部分比例
	{
		double progress_bar_value = (index * DIALOG_PROGRESSBAR_TIMES_NUMBER);//计算比例
		if (progress_bar_value > DIALOG_PROGRESSBAR_MAX_NUMBER)
		{
	...
	void LockSimpleOpposite::receive_unlock_simple_opposite_final(bool is_success)//取反加密文件结束。参数为dll导出类指针
	{
		ui.progressBar->setValue(0);//进度条显示最小值
		progress_bar_change_color = false;//保证只切换一次字体颜色
		ui.progressBar->setStyleSheet("QProgressBar{border: 1px solid #461C0A;background-color: #00FFFFFF;text-align:center;color:#461C0A;}QProgressBar::chunk{background-color:#461C0A;}");
	...
	//加密子线程类函数
	Lock_file_thread::Lock_file_thread()
	{
	...
	void Lock_file_thread::get_data(string *string_array, bool *bool_array, long long *digit_array, int *flag_array, char *char_array)//获取全部数据
	{
		...
		this->digit_array[2] = digit_array[2];
		this->flag_array[0] = flag_array[0];
		this->char_array = char_array;
	}

	void Lock_file_thread::get_target_class_point(Export_LockSimple_virtual_class *point)//获取dll导出类对象指针
	{
		this->target_class_point = point;
	}

	void Lock_file_thread::get_dll_name(HINSTANCE dll_name)//获取dll名
	{
		this->dll_name = dll_name;
	}

	void Lock_file_thread::run()//子线程主函数
	{
		mutex.lock();
		target_class_point->lock_opposite(string_array[0], string_array[1], string_array[2], string_array[3], bool_array[0], string_array[4], bool_array[1], bool_array[2], string_array[5], string_array[6], string_array[7], flag_array[0], digit_array[0], digit_array[1], digit_array[2], bool_array[3], char_array);//加密文件并且返回是否成功。不能使用string，否则因为其会将'\0'作为结束标志，如果恰好有“00”字段则其后面的内容会被直接忽略
		FreeLibrary(dll_name);
		mutex.unlock();
		this->quit();//退出子线程
	}

	Unlock_file_thread::Unlock_file_thread()
	{
	...
	void Unlock_file_thread::run()//子线程主函数
	{
		mutex.lock();
		target_class_point->unlock_opposite(string_array[0], string_array[1], string_array[2], string_array[3], bool_array[0], string_array[4], string_array[7], flag_array[0], digit_array[0], digit_array[1], digit_array[2], bool_array[3], char_array);//解密文件并且返回是否成功。不能使用string，否则因为其会将'\0'作为结束标志，如果恰好有“00”字段则其后面的内容会被直接忽略
		FreeLibrary(dll_name);
		mutex.unlock();
		this->quit();//退出子线程
	}
