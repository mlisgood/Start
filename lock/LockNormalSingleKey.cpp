...
namespace rem
{
	...
	//设置成员函数
	void LockNormalSingleKey::update_unlock_information(QString head_digit, bool have_head_digit, QString key)//更新需要显示在界面上的信息，参数为：加密文件头部特征数字，是否有头部特征数字，密钥
	{
	...
	void LockNormalSingleKey::set_method(QString method)//设置加密方式，参数为加密方式
	{
		this->method = method;
	...
	void LockNormalSingleKey::timer_timeout()//使用2个label用来容纳图片。首先指定一个长时间间隔用来显示图片，此时图片在下层label，上层label使用全透明图片，即将其忽略。时间到期后，改用切换图片透明度所使用的短时间间隔，同时开始修改透明度计数，下层label显示原始图片，不透明度逐渐减小；上层label显示切换后图片，不透明度逐渐增加。当上层label图片透明度为1时，切换结束，图片计数增加1，以新的长时间间隔重新开始下一计时循环
	{
	...
	void LockNormalSingleKey::close_window_include_animation()//包含关闭窗口动画的关闭窗口方法
	{
	...
	void LockNormalSingleKey::combobox_change(const QString &combobox_string)//组合框内容更改后处理
	{

		//设置允许输入的密钥长度
		if (combobox_string == tr("DES"))//DES密钥总长度为64位
		{
			ui.lineEdit_1->setMaxLength(DES::DEFAULT_KEYLENGTH * 2);
			ui.lineEdit_2->setMaxLength(DES::DEFAULT_KEYLENGTH * 2);
			method = tr("DES");
		}
		else if(combobox_string == tr("2-Key 3DES"))//DES_EDE2密钥总长度为128位
		{
			ui.lineEdit_1->setMaxLength(DES_EDE2::DEFAULT_KEYLENGTH * 2);
			ui.lineEdit_2->setMaxLength(DES_EDE2::DEFAULT_KEYLENGTH * 2);
			method = tr("2-Key 3DES");
		}
		else if (combobox_string == tr("3-Key 3DES"))//DES_EDE3密钥总长度为192位
		{
			ui.lineEdit_1->setMaxLength(DES_EDE3::DEFAULT_KEYLENGTH * 2);
			ui.lineEdit_2->setMaxLength(DES_EDE3::DEFAULT_KEYLENGTH * 2);
			method = tr("3-Key 3DES");
		}
		else if (combobox_string == tr("AES-128"))//AES-128密钥总长度为128位
		{
			ui.lineEdit_1->setMaxLength(AES::MIN_KEYLENGTH * 2);
			ui.lineEdit_2->setMaxLength(AES::MIN_KEYLENGTH * 2);
			method = tr("AES-128");
		}
		else if (combobox_string == tr("AES-192"))//AES-192密钥总长度为192位
		{
			ui.lineEdit_1->setMaxLength(FILE_KEY_AES_MIDDLE_KEY_LENGTH * 2);
			ui.lineEdit_2->setMaxLength(FILE_KEY_AES_MIDDLE_KEY_LENGTH * 2);
			method = tr("AES-192");
		}
		else if (combobox_string == tr("AES-256"))//AES-256密钥总长度为256位
		{
			ui.lineEdit_1->setMaxLength(AES::MAX_KEYLENGTH * 2);
			ui.lineEdit_2->setMaxLength(AES::MAX_KEYLENGTH * 2);
			method = tr("AES-256");
	...
	void LockNormalSingleKey::lock_file()//加密文件
	{
		...
		if (file_information.isFile() && *change_result_address)//该文件存在
		{
			...
			if (ui.checkBox_2->isChecked())//是否加密最后一个区块
			{
				bool_array[4] = true;
			}
			else
			...
			Lock_single_file_thread *thread = new Lock_single_file_thread();//处理文件子线程
			...
			emit send_data(string_array, bool_array, char_array, key, method);//发送数据
			using export_class = Export_LockSingle_virtual_class * (*)(void);//定义指向导出函数的指针类型
			...
			if (dll_name != NULL)
			{
				...
				export_class get_class = (export_class)GetProcAddress(dll_name, "export_class_lock_single");//通过导出指定的导出函数来导出需要的类
				if (get_class != NULL)
				{
					Export_LockSingle_virtual_class *target_class_object = get_class();//获得目标类的对象指针
					emit send_target_class_point(target_class_object);//发送dll导出类对象指针
					if (method == tr("DES"))
					{
						connect(target_class_object, SIGNAL(lock_DES_process(double)), this, SLOT(receive_lock_DES_process(double)));//该函数要求的条件：发送者需要继承QObject类并且类中含有Q_OBJECT宏
						connect(target_class_object, SIGNAL(lock_DES_final(bool, char*)), this, SLOT(receive_lock_DES_final(bool, char*)));
					}
					else if (method == tr("2-Key 3DES"))
					{
						connect(target_class_object, SIGNAL(lock_DES_EDE2_process(double)), this, SLOT(receive_lock_DES_EDE2_process(double)));//该函数要求的条件：发送者需要继承QObject类并且类中含有Q_OBJECT宏
						connect(target_class_object, SIGNAL(lock_DES_EDE2_final(bool, char*)), this, SLOT(receive_lock_DES_EDE2_final(bool, char*)));
					}
					else if (method == tr("3-Key 3DES"))
					{
						connect(target_class_object, SIGNAL(lock_DES_EDE3_process(double)), this, SLOT(receive_lock_DES_EDE3_process(double)));//该函数要求的条件：发送者需要继承QObject类并且类中含有Q_OBJECT宏
						connect(target_class_object, SIGNAL(lock_DES_EDE3_final(bool, char*)), this, SLOT(receive_lock_DES_EDE3_final(bool, char*)));
					}
					else if (method == tr("AES-128"))
					{
						connect(target_class_object, SIGNAL(lock_AES_MIN_process(double)), this, SLOT(receive_lock_AES_MIN_process(double)));//该函数要求的条件：发送者需要继承QObject类并且类中含有Q_OBJECT宏
						connect(target_class_object, SIGNAL(lock_AES_MIN_final(bool, char*)), this, SLOT(receive_lock_AES_MIN_final(bool, char*)));
					}
					else if (method == tr("AES-192"))
					{
						connect(target_class_object, SIGNAL(lock_AES_MIDDLE_process(double)), this, SLOT(receive_lock_AES_MIDDLE_process(double)));//该函数要求的条件：发送者需要继承QObject类并且类中含有Q_OBJECT宏
						connect(target_class_object, SIGNAL(lock_AES_MIDDLE_final(bool, char*)), this, SLOT(receive_lock_AES_MIDDLE_final(bool, char*)));
					}
					else if (method == tr("AES-256"))
					{
						connect(target_class_object, SIGNAL(lock_AES_MAX_process(double)), this, SLOT(receive_lock_AES_MAX_process(double)));//该函数要求的条件：发送者需要继承QObject类并且类中含有Q_OBJECT宏
	...
	void LockNormalSingleKey::unlock_file()//解密文件
	{
		File dialog;//这个窗口是从Notepad.exe直接引用的，没有进行任何修改，因此File类的调用方式会比较麻烦
		dialog.setWindowTitle(tr("选择文件"));
		QString file_dialog_file_name = QDir::currentPath();//当前路径，用于获得被打开的文件路径
		...
		if (file_information.isFile() && *change_result_address)//该文件存在
		{
			...
			QString key_text = ui.lineEdit_2->text().toLatin1();//获取密钥内容，转换为支持ASCII的字符集即可
			key_text = key_text.toUpper();//转换为大写
			QByteArray key_text_bytearray = key_text.toLatin1();//转换为QByteArray，转换为支持ASCII的字符集即可
			key_text = key_text_bytearray;//修改lineEdit的内容
			ui.lineEdit_2->setText(key_text);
			if (method == tr("DES"))
			{
				key_text_bytearray = QByteArray::fromHex(key_text_bytearray);//转化为16进制编码
				memmove(key, key_text_bytearray.data(), DES::DEFAULT_KEYLENGTH);//直接复制内存
				UnlockNormalSingleCheck dialog;//创建确认解密信息框
				is_continue = false;//设置是否需要在下一窗口执行完后继续操作
				dialog.set_information(method, &key_text, &lineEdit_text, &bool_array[3], &is_continue);//设定初始值
				dialog.exec();
				if (is_continue)//在解密确认窗口点击确认，接下来进行解密操作
				{
					if (key_text.length() < DES::DEFAULT_KEYLENGTH * 2)
					{
						...
						dialog.setWindowTitle(tr("解密失败了……"));
						dialog.exec();
					}
					else
					{
						update_unlock_information(lineEdit_text, bool_array[3], key_text);//更新窗口内容
						key_text_bytearray = key_text.toLatin1();//将密钥文本转化为二进制数据并赋值
						key_text_bytearray = QByteArray::fromHex(key_text_bytearray);//转化为16进制编码
						memmove(key, key_text_bytearray.data(), DES::DEFAULT_KEYLENGTH);//直接复制内存
						Unlock_single_file_thread *thread = new Unlock_single_file_thread;//处理文件子线程
						connect(thread, &QThread::finished, thread, &QObject::deleteLater);//直接delete子线程极其容易出现各种问题，要想安全删除线程可以使用此方式
						connect(this, SIGNAL(send_data(string*, bool*, char*, char*, QString)), thread, SLOT(get_data(string*, bool*, char*, char*, QString)));
						connect(this, SIGNAL(send_target_class_point(Export_LockSingle_virtual_class *)), thread, SLOT(get_target_class_point(Export_LockSingle_virtual_class *)));
						connect(this, SIGNAL(send_dll_name(HINSTANCE)), thread, SLOT(get_dll_name(HINSTANCE)));
						...
						if (dll_name != NULL)
						{
							...
							if (get_class != NULL)
							{
								Export_LockSingle_virtual_class *target_class_object = get_class();//获得目标类的对象指针
								emit send_target_class_point(target_class_object);//发送dll导出类对象指针
								connect(target_class_object, SIGNAL(unlock_DES_process(double)), this, SLOT(receive_unlock_DES_process(double)));//该函数要求的条件：发送者需要继承QObject类并且类中含有Q_OBJECT宏
			...
			{
				key_text_bytearray = QByteArray::fromHex(key_text_bytearray);//转化为16进制编码
				memmove(key, key_text_bytearray.data(), DES_EDE2::DEFAULT_KEYLENGTH);//直接复制内存
				UnlockNormalSingleCheck dialog;//创建确认解密信息框
				is_continue = false;//设置是否需要在下一窗口执行完后继续操作
				dialog.set_information(method, &key_text, &lineEdit_text, &bool_array[3], &is_continue);//设定初始值
				dialog.exec();
					...
					else
					{
						...
						if (dll_name != NULL)
						{
							...
							if (get_class != NULL)
							{
								Export_LockSingle_virtual_class *target_class_object = get_class();//获得目标类的对象指针
								emit send_target_class_point(target_class_object);//发送dll导出类对象指针
								connect(target_class_object, SIGNAL(unlock_DES_EDE2_process(double)), this, SLOT(receive_unlock_DES_EDE2_process(double)));//该函数要求的条件：发送者需要继承QObject类并且类中含有Q_OBJECT宏
			...
			{
				key_text_bytearray = QByteArray::fromHex(key_text_bytearray);//转化为16进制编码
				memmove(key, key_text_bytearray.data(), DES_EDE3::DEFAULT_KEYLENGTH);//直接复制内存
				UnlockNormalSingleCheck dialog;//创建确认解密信息框
				is_continue = false;//设置是否需要在下一窗口执行完后继续操作
				dialog.set_information(method, &key_text, &lineEdit_text, &bool_array[3], &is_continue);//设定初始值
				dialog.exec();
					...
					else
					{
						...
						if (dll_name != NULL)
						{
							...
							if (get_class != NULL)
							{
								Export_LockSingle_virtual_class *target_class_object = get_class();//获得目标类的对象指针
								emit send_target_class_point(target_class_object);//发送dll导出类对象指针
								connect(target_class_object, SIGNAL(unlock_DES_EDE3_process(double)), this, SLOT(receive_unlock_DES_EDE3_process(double)));//该函数要求的条件：发送者需要继承QObject类并且类中含有Q_OBJECT宏
			...
			{
				key_text_bytearray = QByteArray::fromHex(key_text_bytearray);//转化为16进制编码
				memmove(key, key_text_bytearray.data(), AES::MIN_KEYLENGTH);//直接复制内存
				UnlockNormalSingleCheck dialog;//创建确认解密信息框
				is_continue = false;//设置是否需要在下一窗口执行完后继续操作
				dialog.set_information(method, &key_text, &lineEdit_text, &bool_array[3], &is_continue);//设定初始值
				dialog.exec();
					...
					else
					{
						...
						if (dll_name != NULL)
						{
							...
							if (get_class != NULL)
							{
								Export_LockSingle_virtual_class *target_class_object = get_class();//获得目标类的对象指针
								emit send_target_class_point(target_class_object);//发送dll导出类对象指针
								connect(target_class_object, SIGNAL(unlock_AES_MIN_process(double)), this, SLOT(receive_unlock_AES_MIN_process(double)));//该函数要求的条件：发送者需要继承QObject类并且类中含有Q_OBJECT宏
			...
			{
				key_text_bytearray = QByteArray::fromHex(key_text_bytearray);//转化为16进制编码
				memmove(key, key_text_bytearray.data(), FILE_KEY_AES_MIDDLE_KEY_LENGTH);//直接复制内存
				UnlockNormalSingleCheck dialog;//创建确认解密信息框
				is_continue = false;//设置是否需要在下一窗口执行完后继续操作
				dialog.set_information(method, &key_text, &lineEdit_text, &bool_array[3], &is_continue);//设定初始值
				dialog.exec();
					...
					else
					{
						...
						if (dll_name != NULL)
						{
							...
							if (get_class != NULL)
							{
								Export_LockSingle_virtual_class *target_class_object = get_class();//获得目标类的对象指针
								emit send_target_class_point(target_class_object);//发送dll导出类对象指针
								connect(target_class_object, SIGNAL(unlock_AES_MIDDLE_process(double)), this, SLOT(receive_unlock_AES_MIDDLE_process(double)));//该函数要求的条件：发送者需要继承QObject类并且类中含有Q_OBJECT宏
			...
			{
				key_text_bytearray = QByteArray::fromHex(key_text_bytearray);//转化为16进制编码
				memmove(key, key_text_bytearray.data(), AES::MAX_KEYLENGTH);//直接复制内存
				UnlockNormalSingleCheck dialog;//创建确认解密信息框
				is_continue = false;//设置是否需要在下一窗口执行完后继续操作
				dialog.set_information(method, &key_text, &lineEdit_text, &bool_array[3], &is_continue);//设定初始值
				dialog.exec();
					...
					else
					{
						...
						if (dll_name != NULL)
						{
							...
							if (get_class != NULL)
							{
								Export_LockSingle_virtual_class *target_class_object = get_class();//获得目标类的对象指针
								emit send_target_class_point(target_class_object);//发送dll导出类对象指针
								connect(target_class_object, SIGNAL(unlock_AES_MAX_process(double)), this, SLOT(receive_unlock_AES_MAX_process(double)));//该函数要求的条件：发送者需要继承QObject类并且类中含有Q_OBJECT宏
	...
	void LockNormalSingleKey::receive_lock_DES_process(double index)//DES加密进度条显示。参数为已处理部分比例
	{
	...
	void LockNormalSingleKey::receive_lock_DES_final(bool is_success, char *key)//DES加密结束。参数为dll导出类指针
	{
		...
		if (is_success == true)
		{
			QByteArray key_hex_string(key, DES::DEFAULT_KEYLENGTH);//将密钥转化为QBytearray
			key_hex_string = key_hex_string.toHex().toUpper();//获得十六进制数据，转换为大写
			ui.lineEdit_1->setText((QString)key_hex_string);//主界面显示生成的密钥
		}
		else
		{
			Help dialog;//显示程序信息
	...
	void LockNormalSingleKey::receive_unlock_DES_process(double index)//DES解密文件进度条显示。参数为已处理部分比例
	{
		double progress_bar_value = (index * DIALOG_PROGRESSBAR_TIMES_NUMBER);//计算比例
		if (progress_bar_value > DIALOG_PROGRESSBAR_MAX_NUMBER)
		{
	...
	void LockNormalSingleKey::receive_unlock_DES_final(bool is_success)//DES解密结束。参数为dll导出类指针
	{
		ui.progressBar->setValue(0);//进度条显示最小值
		progress_bar_change_color = false;//保证只切换一次字体颜色
		ui.progressBar->setStyleSheet("QProgressBar{border: 1px solid #461C0A;background-color: #00FFFFFF;text-align:center;color:#461C0A;}QProgressBar::chunk{background-color:#461C0A;}");
	...
	void LockNormalSingleKey::receive_lock_DES_EDE2_process(double index)//DES-EDE2加密进度条显示。参数为已处理部分比例
	{
		receive_lock_DES_process(index);
	}

	void LockNormalSingleKey::receive_lock_DES_EDE2_final(bool is_success, char *key)//DES-EDE2加密结束。参数为dll导出类指针
	{
		ui.progressBar->setValue(0);//进度条显示最小值
		progress_bar_change_color = false;//保证只切换一次字体颜色
		ui.progressBar->setStyleSheet("QProgressBar{border: 1px solid #461C0A;background-color: #00FFFFFF;text-align:center;color:#461C0A;}QProgressBar::chunk{background-color:#461C0A;}");
		if (is_success == true)
		{
			QByteArray key_hex_string(key, DES_EDE2::DEFAULT_KEYLENGTH);//将密钥转化为QBytearray
			key_hex_string = key_hex_string.toHex().toUpper();//获得十六进制数据，转换为大写
			ui.lineEdit_1->setText((QString)key_hex_string);//主界面显示生成的密钥
		}
		else
	...
	void LockNormalSingleKey::receive_unlock_DES_EDE2_process(double index)//DES-EDE2解密文件进度条显示。参数为已处理部分比例
	{
		receive_unlock_DES_process(index);
	}

	void LockNormalSingleKey::receive_unlock_DES_EDE2_final(bool is_success)//DES-EDE2解密结束。参数为dll导出类指针
	{
		receive_unlock_DES_final(is_success);
	}

	void LockNormalSingleKey::receive_lock_DES_EDE3_process(double index)//DES-EDE3加密进度条显示。参数为已处理部分比例
	{
		receive_lock_DES_process(index);
	}

	void LockNormalSingleKey::receive_lock_DES_EDE3_final(bool is_success, char *key)//DES-EDE3加密结束。参数为dll导出类指针
	{
		ui.progressBar->setValue(0);//进度条显示最小值
		progress_bar_change_color = false;//保证只切换一次字体颜色
		ui.progressBar->setStyleSheet("QProgressBar{border: 1px solid #461C0A;background-color: #00FFFFFF;text-align:center;color:#461C0A;}QProgressBar::chunk{background-color:#461C0A;}");
		if (is_success == true)
		{
			QByteArray key_hex_string(key, DES_EDE3::DEFAULT_KEYLENGTH);//将密钥转化为QBytearray
			key_hex_string = key_hex_string.toHex().toUpper();//获得十六进制数据，转换为大写
			ui.lineEdit_1->setText((QString)key_hex_string);//主界面显示生成的密钥
		}
		else
	...
	void LockNormalSingleKey::receive_unlock_DES_EDE3_process(double index)//DES-EDE3解密文件进度条显示。参数为已处理部分比例
	{
		receive_unlock_DES_process(index);
	}

	void LockNormalSingleKey::receive_unlock_DES_EDE3_final(bool is_success)//DES-EDE3解密结束。参数为dll导出类指针
	{
		receive_unlock_DES_final(is_success);
	}

	void LockNormalSingleKey::receive_lock_AES_MIN_process(double index)//AES-128加密进度条显示。参数为已处理部分比例
	{
		receive_lock_DES_process(index);
	}

	void LockNormalSingleKey::receive_lock_AES_MIN_final(bool is_success, char *key)//AES-128加密结束。参数为dll导出类指针
	{
		ui.progressBar->setValue(0);//进度条显示最小值
		progress_bar_change_color = false;//保证只切换一次字体颜色
		ui.progressBar->setStyleSheet("QProgressBar{border: 1px solid #461C0A;background-color: #00FFFFFF;text-align:center;color:#461C0A;}QProgressBar::chunk{background-color:#461C0A;}");
		if (is_success == true)
		{
			QByteArray key_hex_string(key, AES::MIN_KEYLENGTH);//将密钥转化为QBytearray
			key_hex_string = key_hex_string.toHex().toUpper();//获得十六进制数据，转换为大写
			ui.lineEdit_1->setText((QString)key_hex_string);//主界面显示生成的密钥
		}
		else
	...
	void LockNormalSingleKey::receive_unlock_AES_MIN_process(double index)//AES-128解密文件进度条显示。参数为已处理部分比例
	{
		receive_unlock_DES_process(index);
	}

	void LockNormalSingleKey::receive_unlock_AES_MIN_final(bool is_success)//AES-128解密结束。参数为dll导出类指针
	{
		receive_unlock_DES_final(is_success);
	}

	void LockNormalSingleKey::receive_lock_AES_MIDDLE_process(double index)//AES-256加密进度条显示。参数为已处理部分比例
	{
		receive_lock_DES_process(index);
	}

	void LockNormalSingleKey::receive_lock_AES_MIDDLE_final(bool is_success, char *key)//AES-256加密结束。参数为dll导出类指针
	{
		ui.progressBar->setValue(0);//进度条显示最小值
		progress_bar_change_color = false;//保证只切换一次字体颜色
		ui.progressBar->setStyleSheet("QProgressBar{border: 1px solid #461C0A;background-color: #00FFFFFF;text-align:center;color:#461C0A;}QProgressBar::chunk{background-color:#461C0A;}");
		if (is_success == true)
		{
			QByteArray key_hex_string(key, FILE_KEY_AES_MIDDLE_KEY_LENGTH);//将密钥转化为QBytearray
			key_hex_string = key_hex_string.toHex().toUpper();//获得十六进制数据，转换为大写
			ui.lineEdit_1->setText((QString)key_hex_string);//主界面显示生成的密钥
		}
		else
	...
	void LockNormalSingleKey::receive_unlock_AES_MIDDLE_process(double index)//AES-256解密文件进度条显示。参数为已处理部分比例
	{
		receive_unlock_DES_process(index);
	}

	void LockNormalSingleKey::receive_unlock_AES_MIDDLE_final(bool is_success)//AES-256解密结束。参数为dll导出类指针
	{
		receive_unlock_DES_final(is_success);
	}

	void LockNormalSingleKey::receive_lock_AES_MAX_process(double index)//AES-256加密进度条显示。参数为已处理部分比例
	{
		receive_lock_DES_process(index);
	}

	void LockNormalSingleKey::receive_lock_AES_MAX_final(bool is_success, char *key)//AES-256加密结束。参数为dll导出类指针
	{
		ui.progressBar->setValue(0);//进度条显示最小值
		progress_bar_change_color = false;//保证只切换一次字体颜色
		ui.progressBar->setStyleSheet("QProgressBar{border: 1px solid #461C0A;background-color: #00FFFFFF;text-align:center;color:#461C0A;}QProgressBar::chunk{background-color:#461C0A;}");
		if (is_success == true)
		{
			QByteArray key_hex_string(key, AES::MAX_KEYLENGTH);//将密钥转化为QBytearray
			key_hex_string = key_hex_string.toHex().toUpper();//获得十六进制数据，转换为大写
			ui.lineEdit_1->setText((QString)key_hex_string);//主界面显示生成的密钥
		}
		else
	...
	void LockNormalSingleKey::receive_unlock_AES_MAX_process(double index)//AES-256解密文件进度条显示。参数为已处理部分比例
	{
		receive_unlock_DES_process(index);
	}

	void LockNormalSingleKey::receive_unlock_AES_MAX_final(bool is_success)//AES-256解密结束。参数为dll导出类指针
	{
		receive_unlock_DES_final(is_success);
	}
	...
	void Lock_single_file_thread::get_data(string *string_array, bool *bool_array, char *char_array, char *key, QString method)//获取全部数据
	{
	...
	void Lock_single_file_thread::get_target_class_point(Export_LockSingle_virtual_class *point)//获取dll导出类对象指针
	{
		this->target_class_point = point;
	}

	void Lock_single_file_thread::get_dll_name(HINSTANCE dll_name)//获取dll名
	{
		this->dll_name = dll_name;
	}

	void Lock_single_file_thread::run()//子线程主函数
	{
		mutex.lock();
		if (method == tr("DES"))//DES加密
		{
			target_class_point->lock_DES(string_array[0], string_array[1], string_array[2], string_array[3], bool_array[0], string_array[4], bool_array[1], bool_array[2], string_array[5], string_array[6], bool_array[3], char_array, key, bool_array[4]);//加密文件并且返回是否成功。不能使用string，否则因为其会将'\0'作为结束标志，如果恰好有“00”字段则其后面的内容会被直接忽略。由于DES加密需要补足不足一个加密单位的部分，故选择不加密最后不足一个加密单位的数据块，否则会造成明文和密文长度不相同
		}
		else if (method == tr("2-Key 3DES"))//DES_EDE2加密
		{
		...
		else if (method == tr("3-Key 3DES"))//DES_EDE3加密
		{
		...
		else if (method == tr("AES-192"))//AES-192加密
		{
		...
		else if (method == tr("AES-256"))//AES-256加密
		{
	...
	void Unlock_single_file_thread::run()//子线程主函数
	{
		mutex.lock();
		if (method == tr("DES"))//DES加密
		{
			target_class_point->unlock_DES(string_array[0], string_array[1], string_array[2], string_array[3], bool_array[0], string_array[4], bool_array[3], char_array, key, bool_array[4]);//解密文件并且返回是否成功。不能使用string，否则因为其会将'\0'作为结束标志，如果恰好有“00”字段则其后面的内容会被直接忽略
		}
		else if (method == tr("2-Key 3DES"))//DES_EDE2加密
		{
			target_class_point->unlock_DES_EDE2(string_array[0], string_array[1], string_array[2], string_array[3], bool_array[0], string_array[4], bool_array[3], char_array, key, bool_array[4]);//解密文件并且返回是否成功。不能使用string，否则因为其会将'\0'作为结束标志，如果恰好有“00”字段则其后面的内容会被直接忽略
		}
		else if (method == tr("3-Key 3DES"))//DES_EDE3加密
		{
			target_class_point->unlock_DES_EDE3(string_array[0], string_array[1], string_array[2], string_array[3], bool_array[0], string_array[4], bool_array[3], char_array, key, bool_array[4]);//解密文件并且返回是否成功。不能使用string，否则因为其会将'\0'作为结束标志，如果恰好有“00”字段则其后面的内容会被直接忽略
		}
		else if (method == tr("AES-128"))//AES_MIN加密
		{
			target_class_point->unlock_AES_MIN(string_array[0], string_array[1], string_array[2], string_array[3], bool_array[0], string_array[4], bool_array[3], char_array, key, bool_array[4]);//解密文件并且返回是否成功。不能使用string，否则因为其会将'\0'作为结束标志，如果恰好有“00”字段则其后面的内容会被直接忽略
		}
		else if (method == tr("AES-192"))//AES_MIN加密
		{
			target_class_point->unlock_AES_MIDDLE(string_array[0], string_array[1], string_array[2], string_array[3], bool_array[0], string_array[4], bool_array[3], char_array, key, bool_array[4]);//解密文件并且返回是否成功。不能使用string，否则因为其会将'\0'作为结束标志，如果恰好有“00”字段则其后面的内容会被直接忽略
		}
		else if (method == tr("AES-256"))//AES_MIN加密
		{
			target_class_point->unlock_AES_MAX(string_array[0], string_array[1], string_array[2], string_array[3], bool_array[0], string_array[4], bool_array[3], char_array, key, bool_array[4]);//解密文件并且返回是否成功。不能使用string，否则因为其会将'\0'作为结束标志，如果恰好有“00”字段则其后面的内容会被直接忽略
		}
		else
		{
			target_class_point->unlock_DES(string_array[0], string_array[1], string_array[2], string_array[3], bool_array[0], string_array[4], bool_array[3], char_array, key, bool_array[4]);//解密文件并且返回是否成功。不能使用string，否则因为其会将'\0'作为结束标志，如果恰好有“00”字段则其后面的内容会被直接忽略
