...
namespace rem
{
	...
	File::File(QWidget *parent) : QDialog(parent)
	{
		...
		//加载按钮样式
		ui.pushButton_exit->setStyleSheet("QPushButton{border-image: url(:/picture_lock/dialogicon/picture_lock/icon/dialog_filedialog_exit_normal.png);}""QPushButton:hover{border-image: url(:/picture_lock/dialogicon/picture_lock/icon/dialog_filedialog_exit_pressed.png);}");
	...
	void File::set_button_save_or_open(int status)//设置按钮
	{
		...
		if (status == 0)
		{
			ui.pushButton_open->setStyleSheet("QPushButton{border-image: url(:/picture_lock/dialogicon/picture_lock/icon/dialog_filedialog_open_normal.png);}""QPushButton:hover{border-image: url(:/picture_lock/dialogicon/picture_lock/icon/dialog_filedialog_open_pressed.png);}");
		}
		else if (status == 1)
		{
			ui.pushButton_open->setStyleSheet("QPushButton{border-image: url(:/picture_lock/dialogicon/picture_lock/icon/dialog_filedialog_save_normal.png);}""QPushButton:hover{border-image: url(:/picture_lock/dialogicon/picture_lock/icon/dialog_filedialog_save_pressed.png);}");
		}
		else
		{
	...
	void File::get_tree_view_selected_file_name(const QModelIndex &index)//文件路径
	{
		if (file_tree->fileInfo(index).isDir())//是路径
		{
			...
			else if (open_or_save == 1)//保存
			{
				...
				file_name += tr("/") + FILE_LOCK_DOUBLE_KEY_DEFAULT_NAME_STRING;
