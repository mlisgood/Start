...
namespace rem
{
	...
	//构造与析构函数
	UnlockSimpleAddCheck::UnlockSimpleAddCheck(QWidget *parent) : QDialog(parent)
	{
		...
		connect(ui.pushButton_look, SIGNAL(clicked()), this, SLOT(look_set()));//查看设置
		...
		ui.pushButton_look->setStyleSheet("QPushButton{border-image: url(:/icon/mainicon/picture_lock/icon/lock_button_normal_13.png);}""QPushButton:hover{border-image: url(:/icon/mainicon/picture_lock/icon/lock_button_pressed_13.png);}");
	...
	UnlockSimpleAddCheck::~UnlockSimpleAddCheck()
	{
	...
	void UnlockSimpleAddCheck::mousePressEvent(QMouseEvent *event)
	{
	...
	//若鼠标左键被按下，则移动窗体位置
	void UnlockSimpleAddCheck::mouseMoveEvent(QMouseEvent *event)
	{
		if (m_bPressed)
		{
	...
	//鼠标未被按下，则相应变量为false
	void UnlockSimpleAddCheck::mouseReleaseEvent(QMouseEvent *event)
	{
		Q_UNUSED(event);
		m_bPressed = false;
	}
	...
	void UnlockSimpleAddCheck::keyPressEvent(QKeyEvent *event)
	{
	...
	void UnlockSimpleAddCheck::paintEvent(QPaintEvent *)
	{
	...
	//设置成员函数
	void UnlockSimpleAddCheck::set_information(int *direction, long long *head_offset, long long *tail_offset, int *cycle, QString *head_digit, bool *have_head_digit, bool *is_continue, int *operate_count)//设置需要显示在界面上的信息，参数为：加密方向（1：顺序加密，2：倒序加密，3：双向加密），头部忽略字节，尾部忽略字节，循环周期，加密文件头部特征数字，是否有头部特征数字，是否确认解密，加减数量
	{
		long_long_information[0] = head_offset;//头部忽略字节
		long_long_information[1] = tail_offset;//尾部忽略字节
		extra_int_information[DIALOG_LOCK_SIMPLE_ADD_MAX_UNIT_NUMBER] = cycle;//处理周期
		char_point_information = head_digit;//头部特征数字
		int_information = direction;//加密方向
		bool_information = have_head_digit;//是否有头部特征数字
		for (int index = 0; index < DIALOG_LOCK_SIMPLE_ADD_MAX_UNIT_NUMBER; index++)//加减数量
		{
			extra_int_information[index] = &operate_count[index];
		}
		...
		set_information_data(*direction, *head_offset, *tail_offset, *cycle, *head_digit, *have_head_digit, operate_count);//先将数据存入该类自带的成员变量中，如果导入密钥文件再更新
	}

	void UnlockSimpleAddCheck::set_information_data(int direction, long long head_offset, long long tail_offset, int cycle, QString head_digit, bool have_head_digit, int *operate_count)//设置需要显示在界面上的信息，但不再传入指针，供之后使用。如果在之后使用指针传递，则会造成指针指向的地址改变，那么在确认后无法修改主窗口中相关属性的值
	{
		long_long_information_data[0] = head_offset;//头部忽略字节
		long_long_information_data[1] = tail_offset;//尾部忽略字节
		extra_int_information_data[DIALOG_LOCK_SIMPLE_ADD_MAX_UNIT_NUMBER] = cycle;//处理周期
		...
		for (int index = 0; index < DIALOG_LOCK_SIMPLE_ADD_MAX_UNIT_NUMBER; index++)//加减数量
		{
			extra_int_information_data[index] = operate_count[index];
		}
	...
	void UnlockSimpleAddCheck::set_apply()//设置生效
	{
		...
		*int_information = int_information_data;
		for (int index = 0; index < DIALOG_LOCK_SIMPLE_ADD_MAX_UNIT_NUMBER + 1; index++)
		{
			*extra_int_information[index] = extra_int_information_data[index];
		}
		*is_continue = true;//点击确认，主界面接下来进行解密操作
		close_window_include_animation();//关闭窗口
	}

	void UnlockSimpleAddCheck::cancel_apply()//设置取消
	{
		*is_continue = false;//点击取消，不执行任何操作
		close_window_include_animation();//关闭窗口
	}

	void UnlockSimpleAddCheck::import_set()//导入设置
	{
		...
		if (file_information.isFile() && *change_result_address)//该文件存在
		{
			...
			else
			{
				...
				else
				{
					...
					if (type_number_digit != FILE_KEY_WRITE_BINARY_TYPE_2_NUMBER)//不是简单加减
					{
						Help dialog;//显示程序信息
						dialog.set_dialog_information_path(DIALOG_INFORMATION_OPEN_WRONG_KEYFILE_PATH_STRING);
						dialog.setWindowTitle(tr("打开了错误的文件……"));
					...
					else
					{
						...
						keyfile.clear();//清空文件流，否则可能无法定位
						keyfile.seekg(seek_offset, ios_base::beg);//移动文件指针
						keyfile.read((char*)&extra_int_information_data[DIALOG_LOCK_SIMPLE_ADD_MAX_UNIT_NUMBER], sizeof(int));//读取循环周期
						seek_offset += sizeof(int);
						for (int index = 0; index < extra_int_information_data[DIALOG_LOCK_SIMPLE_ADD_MAX_UNIT_NUMBER]; index++)
						{
							keyfile.read((char*)&extra_int_information_data[index], sizeof(int));//读取加减单位
							seek_offset += sizeof(int);
						}
						seek_offset += FILE_KEYFILR_INTERVAL_NUMBER_HEX_STRING_SIZE_NUMBER;//分隔符
						keyfile.clear();//清空文件流，否则可能无法定位
						...
						keyfile.close();
						set_information_data(int_information_data, long_long_information_data[0], long_long_information_data[1], extra_int_information_data[DIALOG_LOCK_SIMPLE_ADD_MAX_UNIT_NUMBER], char_point_information_data, bool_information_data, extra_int_information_data);//传递修改后信息，此时不能再用指针传值，否则会造成相应变量指向地址的改变
					}
				}
				delete magic_number;
			}
	...
	void UnlockSimpleAddCheck::look_set()//查看设置
	{
		UnlockSimpleAddCheckDigit dialog;
		dialog.set_information(extra_int_information_data);
		dialog.exec();
	}

	void UnlockSimpleAddCheck::close_window_include_animation()//包含关闭窗口动画的关闭窗口方法
	{
