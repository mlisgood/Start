...
namespace rem
{
	...
	private:
		...
		Ui::UnlockNormalSingleCheckClass ui;//窗口
	...
	public:
		...
		//设置成员函数
		void set_information(QString method, QString *key, QString *head_digit, bool *have_head_digit, bool *is_continue);//设置需要显示在界面上的信息，参数为：加密方法，由二进制数据转化为文本的密钥，加密文件头部特征数字，是否有头部特征数字，是否确认解密
		void set_information_data(QString key, QString head_digit, bool have_head_digit);//设置需要显示在界面上的信息，但不再传入指针，供之后使用。如果在之后使用指针传递，则会造成指针指向的地址改变，那么在确认后无法修改主窗口中相关属性的值
