...
namespace rem
{
	...
	private:
		...
		long long digit_array[2];//存放long long型数值信息，按顺序分为：头部处理起点，尾部处理起点，循环周期
		char char_array[5];//存放char*字符串信息，包含头部特征数字。预留一个字节给'\0'字符
		int flag_array[1];//存放int型标记信息，按顺序分为：加密方向（1代表顺序，2代表倒序，3代表双方向）
		int extra_digit_array[17];//存放int型加密加减数量，最大16个，最后一个参数指定一个循环内的单位数目
		int LockSimpleAdd_already_send_size;//已经发送大小
		int LockSimpleAdd_file_size;//文件大小
	...
	public:
		...
		//设置成员函数
		void update_unlock_information(int direction, long long head_offset, long long tail_offset, int cycle, QString head_digit, bool have_head_digit, int *operate_count);//更新需要显示在界面上的信息，参数为：加密方向（1：顺序加密，2：倒序加密，3：双向加密），头部忽略字节，尾部忽略字节，循环周期，加密文件头部特征数字，是否有头部特征数字，加减数量
	...
	signals:
		...
		void send_data(string *string_array, bool *bool_array, long long *digit_array, int *flag_array, int *extra_digit_array, char* char_array);//发送数据
	...
	public slots:
		...
		void digit_set();//设置加减数量
		void receive_lock_simple_add_process(double index);//加减加密文件进度条显示。参数为已处理部分比例
		void receive_lock_simple_add_final(bool is_success);//加减加密文件结束。参数为加密是否成功
		void receive_unlock_simple_add_process(double index);//加减解密文件进度条显示。参数为已处理部分比例
		void receive_unlock_simple_add_final(bool is_success);//加减解密文件结束。参数为加密是否成功
	};

	class Lock_add_file_thread : public QThread//加密文件子线程
	{
		Q_OBJECT
	...
	protected:
		...
		bool bool_array[4];//存放布尔值信息，按顺序分为：是否输出历史文件，是否输出密钥文件，是否输出二进制密钥文件，是否有头部特征数字
		long long digit_array[2];//存放long long型数值信息，按顺序分为：头部处理起点，尾部处理起点
		int flag_array[2];//存放int型标记信息，按顺序分为：加密方向，循环周期
		int extra_digit_array[16];//存放int型加密加减数量，最大16个
	...
	public:
		...
		Lock_add_file_thread();//构造函数
		~Lock_add_file_thread();//析构函数

	public slots:

		void get_data(string *string_array, bool *bool_array, long long *digit_array, int *flag_array, int *extra_digit_array, char *char_array);//获取全部数据
	...
	class Unlock_add_file_thread : public Lock_add_file_thread//解密文件子线程
	{
		Q_OBJECT
	...
	public:
		...
		Unlock_add_file_thread();//构造函数
		~Unlock_add_file_thread();//析构函数
