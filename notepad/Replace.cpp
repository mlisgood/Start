...
namespace rem
{
	...
	void Replace::find_text_last()//向前替换文本
	{
		QString find_text = ui.lineEdit_1->text();//需要查找的文本
		QString replace_text = ui.lineEdit_2->text();//替换成的文本
		...
		if (is_success)//若成功则替换，否则报错
		{
			textedit_find->insertPlainText(replace_text);//得到当前text的光标。因为替换时插入文本后光标会自动移动到文本后面，如果不进行处理会导致如果查找字符串是替换字符串的字串，每次会替换最后一个字符串
	...
	void Replace::find_text_next()//向后替换文本
	{
		QString find_text = ui.lineEdit_1->text();//需要查找的文本
		QString replace_text = ui.lineEdit_2->text();//替换成的文本
		bool is_success;//判断是否查找成功
	...
	void Replace::find_text_all()//全部替换文本
	{
		QString find_text = ui.lineEdit_1->text();//需要查找的文本
		QString replace_text = ui.lineEdit_2->text();//替换成的文本
		bool is_success = true;//判断是否查找成功
		QTextCursor current_cursor = textedit_find->textCursor();//记录光标位置
		textedit_find->moveCursor(QTextCursor::Start, QTextCursor::MoveAnchor);//将光标移动到文本开始位置
		while (is_success)//如果上次替换成功，则再查找一次，直到替换完毕。
		{
			//枚举情况并分别处理
			if (ui.checkBox_1->isChecked() == false && ui.checkBox_2->isChecked() == false)//默认
			{
		...
		textedit_find->setTextCursor(current_cursor);//恢复光标位置
		Help dialog;//替换完毕信息
		dialog.setWindowTitle(tr("替换完成！"));
		dialog.set_dialog_information_path(DIALOG_INFORMATION_REPLACE_ALL_PICTURE_PATH_STRING);
		dialog.exec();//如果不加这一句，对话框在创建后会被立即析构
	}

	void Replace::close_window_include_animation()//包含关闭窗口动画的关闭窗口方法
	{
